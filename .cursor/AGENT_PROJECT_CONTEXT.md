...
## 附件「本机有、他人无」常见根因（2026-06-02）

1. **未点「提交上传」**：仅选文件不会写云端。
2. **右上角未「云端已同步」**：初始化/网络失败时 `requireCloud` 上传会拒绝。
3. **整站 JSON >5.5MB**：PUT 失败；上传方本地仍有数据，他人拉不到。
4. **单文件 >8MB**：只同步文件名，无 `dataUrl`。
5. **合并曾抹掉 dataUrl**：入站/冲突合并时较新记录若 `dataUrl` 为空会覆盖有内容的记录——已在 `collabMerge.mergeAttachmentRecord` 修复。
6. **操作认知**：留言区选文件 ≠ 任务附件区；须对应区域提交。
