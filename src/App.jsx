...
function App() {
  ...
  const pendingLocalRevisionRef = useRef(0);
  ...
  const applyLocalState = (nextState) => {
    ...
    flushToLocalStorage(normalizedState);
  ...
  const persist = async (next, { requireCloud = false } = {}) => {
    const baseState = dataRef.current;
    const normalizedNext = normalizeState(typeof next === "function" ? next(baseState) : next);
    const revisionAtStart = localRevisionRef.current;
    let writeRevision = revisionAtStart;
    ...
    persistQueueRef.current = queuedSave.catch(() => {});
  ...
  const completeTutorial = () => {
    ...
    persist((current) => ({
      ...current,
      users: current.users.map((u) => (u.id === currentUser.id ? { ...u, tutorialDone: true } : u)),
    }));
  ...
  const updateTask = (taskId, patch, action) => {
    ...
    persist((current) => {
      ...
      return { ...current, tasks, globalLogs: globalLog ? [globalLog, ...current.globalLogs] : current.globalLogs };
    });
  };
  ...
  const handleRegister = (event) => {
    ...
    persist((current) => ({ ...current, users: [...current.users, newUser] }));
    setAuthError("");
  ...
  const saveTask = (formTask) => {
    ...
    if (formTask.id) {
      ...
      persist({ ...current, tasks, globalLogs: globalLog ? [globalLog, ...current.globalLogs] : current.globalLogs });
    } else {
      const moduleTasks = current.tasks.filter((task) => task.module === formTask.module);
      const newTask = {
        id: makeId("task"),
        ...formTask,
        ...
        dashboardOrder: current.tasks.length + 1,
        attachments: [],
        comments: [],
        logs: [],
      };
      const { task: loggedNewTask, log } = addLogToTask(newTask, `${actor} 新增了任务`, at);
      persist({ ...current, tasks: [...current.tasks, loggedNewTask], globalLogs: [log, ...current.globalLogs] });
  ...
  const deleteTask = (taskId) => {
    ...
    persist({
      ...
      trash: [loggedDeleted, ...(current.trash || [])],
  ...
  const restoreTask = (taskId) => {
    ...
    const moduleMax = Math.max(
      0,
      ...current.tasks.filter((t) => t.module === trashItem.module).map((t) => Number(t.order || 0)),
    );
    const dashboardMax = Math.max(0, ...current.tasks.map((t) => Number(t.dashboardOrder || 0)));
    const restored = {
      ...trashItem,
      deletedAt: undefined,
      order: moduleMax + 1,
    ...
    persist({
      ...current,
      tasks: [...current.tasks, loggedRestored],
  ...
  const deleteTaskAttachment = (taskId, attachmentId) => {
    ...
    persist({
      ...current,
      tasks,
      attachmentTrash: [removed, ...(current.attachmentTrash || [])],
      globalLogs: [log, ...(current.globalLogs || [])],
    });
  };
  ...
  const deleteLogEntry = (taskId, logId) => {
    ...
    persist({
      ...current,
      tasks,
      globalLogs: (current.globalLogs || []).filter((log) => log.id !== logId),
    });
  };
  ...
  const moveTask = (taskId, direction) => {
    ...
    if (!moving) return;
    const moduleTasks = current.tasks
  ...
  const reorderDashboardTask = (targetId) => {
    ...
    persist({ ...current, tasks, globalLogs: [log, ...current.globalLogs] });
  ...
  useEffect(() => {
    ...
    const applyRemoteBundle = (remoteState, updatedAt, detailMsg) => {
      ...
      if (beforeRevision !== stateRevision(merged)) {
        applyLocalState(merged);
        if (hasPendingLocalWrite) pendingLocalRevisionRef.current = localRevisionRef.current;
      }
    ...
    const run = async () => {
      ...
      try {
        ...
        if (hasCoreStateShape(remoteState) && !isEmptyCoreState(remoteState)) {
          const normalizedRemote = normalizeState(remoteState);
          const { state: mergedOnce } = mergeInboundCollaborativeState(dataRef.current, normalizedRemote);
